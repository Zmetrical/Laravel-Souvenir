    ...
    <div class="el-grid">
      @foreach($grouped as $groupName => $items)
        ...
        <div class="el-card {{ !$el->is_active ? 'inactive' : '' }}">
          ...
          {{-- Cube swatch — square with rounded corners instead of circle --}}
          <div class="swatch"
               style="background:{{ $el->color ?? '#DDD' }};border-radius:8px;position:relative;">
            @if($el->detail_color && $el->shape && str_contains($el->shape, 'cube'))
              {{-- tiny detail dot --}}
              <span style="width:10px;height:10px;border-radius:50%;
                           background:{{ $el->detail_color }};
                           position:absolute;bottom:6px;right:6px;"></span>
            @endif
          </div>
