  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .auth-aside::before {
      ...
      background:
        radial-gradient(ellipse 55% 50% at 20% 40%, rgba(26,200,196,.2) 0%, transparent 70%),
        radial-gradient(ellipse 50% 55% at 80% 70%, rgba(255,95,160,.15) 0%, transparent 70%);
      pointer-events: none;
    }
    ...
    .frow { display: flex; flex-direction: column; gap: 6px; margin-bottom: 18px; }
    ...
    .fhint { font-size: .7rem; font-weight: 700; color: var(--ink-md); }
    ...
    .fbtn:hover { background: var(--teal-dk); transform: translateY(-2px); box-shadow: 0 10px 28px rgba(26,200,196,.38); }
    .fbtn:active { transform: scale(.98); }

    .auth-back { text-align: center; margin-top: 20px; font-size: .82rem; font-weight: 700; color: var(--ink-md); }
    .auth-back a { color: var(--pink); font-weight: 900; text-decoration: none; }
...
<div class="auth-wrap">
  ...
  <div class="auth-aside">
    ...
    <div class="aside-body">
      <h2 class="aside-title">Almost there.<br><span>New password, new start.</span></h2>
      <p class="aside-sub">
  ...
  <div class="auth-main">
    <div class="auth-card">
      ...
      <p class="auth-card-sub">
        Pick something secure — at least 8 characters. You won't need the reset link again after this.
      </p>

      <form method="POST" action="{{ route('password.update') }}" novalidate>
        @csrf

        {{-- Hidden token from the email link --}}
        <input type="hidden" name="token" value="{{ $token }}"/>

        {{-- Email (pre-filled from the reset link query string) --}}
        <div class="frow">
        ...
        <button type="submit" class="fbtn">Reset Password →</button>
      </form>
