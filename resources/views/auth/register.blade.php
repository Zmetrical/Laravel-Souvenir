...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .auth-footer a { color: var(--teal-dk); font-weight: 900; text-decoration: none; }
...
<div class="auth-page">
  ...
  <div class="auth-card">
    ...
    <p class="auth-subtitle">Save your designs and track your orders.</p>
    ...
    <form method="POST" action="{{ route('register') }}" novalidate>
      ...
      <div style="display: flex; gap: 16px; margin-bottom: 16px;">
        <div class="form-group" style="margin-bottom: 0;">
          ...
          <input class="form-control {{ $errors->has('first_name') ? 'is-error' : '' }}" type="text" id="first_name" name="first_name" value="{{ old('first_name') }}" required autofocus/>
          @error('first_name')<span class="error-msg">{{ $message }}</span>@enderror
        </div>
        <div class="form-group" style="margin-bottom: 0;">
          <label class="form-label" for="last_name">Last Name</label>
          <input class="form-control {{ $errors->has('last_name') ? 'is-error' : '' }}" type="text" id="last_name" name="last_name" value="{{ old('last_name') }}" required/>
      ...
      <div class="form-group">
        ...
        <input class="form-control {{ $errors->has('email') ? 'is-error' : '' }}" type="email" id="email" name="email" value="{{ old('email') }}" required/>
